"""Enqueue transcription work as Cloud Tasks.
...
Cloud Tasks, rather than a direct Pub/Sub push to the worker, is what gives us the
capped retry budget and the backlog re-enqueue story; see
``spec-local/plan/04-trigger.md`` (decision 6).
...
# Path on the worker service a task POSTs to; joined onto tasks_worker_url.
...
async def enqueue_transcription(record_id: str) -> None:
    ...
    no-op.
    """
    await asyncio.to_thread(_create_task, record_id)


def _create_task(record_id: str) -> None:
    ...
    target_url = worker_url.rstrip("/") + _TRANSCRIBE_PATH
    http_request: dict = {
        "http_method": tasks_v2.HttpMethod.POST,
        "url": target_url,
        "headers": {"Content-Type": "application/json"},
        "body": json.dumps({"recordId": record_id}).encode(),
    }
    ...
    try:
        ...
        raise TaskQueueError(f"Failed to enqueue transcription task: {type(exc).__name__}") from exc
