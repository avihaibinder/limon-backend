...
# All tag routes operate on the authenticated user's tags only; a foreign
# tag id is answered with 404 rather than 403 to avoid leaking existence.


async def _get_own_tag_or_404(session: SessionDep, current_user_id: str, tag_id: str) -> Tag:
    tag = await tags_service.get_tag(session, tag_id)
    if tag is None or tag.user_id != current_user_id:
        raise HTTPException(
...
async def update_tag(
    session: SessionDep, current_user: CurrentUserDep, tag_id: str, payload: TagUpdate
) -> Tag:
